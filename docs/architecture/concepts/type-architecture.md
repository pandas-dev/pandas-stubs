...
guide**: unlike the generated chronicles it is not derived from the committed snapshot,
but reflects the current `.pyi` stubs and is kept in sync with them by the validator
`scripts/check_container_hierarchy.py` (wired into the `architecture` CI job).

The records it links to — the ADRs in [`../decisions/`](../decisions/), the
[subsystem guides](../subsystems/), and the [type matrices](../matrices/) — are the
evidence-backed history behind these rules; this document is the current, normative
statement of them.
...
- **Reality vs. the old guide**: `DataFrame`'s forward arithmetic is mostly
  `other: Any` — only `__floordiv__` and `__truediv__` name a concrete union
...
standard composite type aliases in [`pandas-stubs/core/base.pyi`](../../../pandas-stubs/core/base.pyi)
(L169–234). The construction pattern is unchanged from the original design (see
[ADR-0014](../decisions/adr-0014-shared-typing-architecture.md)):
...
> **Correction vs. the earlier `docs/architecture.md`.** The old text listed
> `ScalarArrayIndexDateTime` / `ScalarArrayIndexSeriesDateTime` and
> `ScalarArrayIndexPeriod` / `ScalarArrayIndexSeriesPeriod`. Those aliases **do not
> exist** in the current stubs: datetime and `Period` operands are spelled **inline** in
> the affected overloads rather than via the alias system. The "Just*" family
> (`JustInt` / `JustFloat` / `JustComplex`, wrapping bare `int`/`float`/`complex` via the
> `Just` protocol) is the finer-grained split the old table did not surface.

### Where the aliases are (and are not) used

The alias system is not applied uniformly to every operator:

- `Index.__add__` (`indexes/base.pyi` L642–697) and `Series.__add__` (`series.pyi`
...
The [Series arithmetic algebra matrix](../matrices/01-series-arithmetic-algebra-matrix.md)
is the generated cross-reference of the `Series` overloads; the
[Series arithmetic subsystem guide](../subsystems/02-series-arithmetic-and-operators.md)
narrates how these signatures were standardized over time.
...
parameter as positional-only, matching the CPython runtime protocol (see
[ADR-0010](../decisions/adr-0010-pep570-positional-only-parameters.md)):
...
bounds (see [ADR-0009](../decisions/adr-0009-protocols-for-type-discrimination.md)):
...
[ADR-0008](../decisions/adr-0008-multi-checker-ignore-standards.md)):
...
(see [ADR-0006](../decisions/adr-0006-multi-checker-testing-harness.md) and
[ADR-0007](../decisions/adr-0007-negative-type-testing-pattern.md)):
