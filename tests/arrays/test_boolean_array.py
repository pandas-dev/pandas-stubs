...
def test_construction_dtype(
    ...
    check(pd.array([True, *data, np.nan], dtype), BooleanArray, dtype_na)
    check(pd.array([np.bool(True), *data, np.nan], dtype), BooleanArray, dtype_na)
...
def test_constructor() -> None:
    ...
    if TYPE_CHECKING_INVALID_USAGE:
        ...
        _i = BooleanArray(pd.Index([1]), np.array([False]))  # type: ignore[arg-type] # pyright: ignore[reportArgumentType]
        _s = BooleanArray(pd.Series([1]), np.array([False]))  # type: ignore[arg-type] # pyright: ignore[reportArgumentType]


def test_dtype() -> None:
    arr = pd.array([True])
    check(assert_type(arr.dtype, pd.BooleanDtype), pd.BooleanDtype)
