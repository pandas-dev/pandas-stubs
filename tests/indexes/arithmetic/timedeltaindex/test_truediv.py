...
def test_truediv_py_sequence(left: pd.TimedeltaIndex) -> None:
...
def test_truediv_numpy_array(left: pd.TimedeltaIndex) -> None:
    ...
    check(assert_type(d / left, "npt.NDArray[np.float64]"), pd.Index, np.floating)


def test_truediv_pd_scalar(left: pd.TimedeltaIndex) -> None:
...
def test_truediv_pd_index(left: pd.TimedeltaIndex) -> None:
    """Test pd.TimedeltaIndex / pandas pd.Indexes"""
    b = pd.Index([True])
    i = pd.Index([2])
    f = pd.Index([1.5])
    c = pd.Index([1.7j])
