...
def test_truediv_numpy_array() -> None:
    ...
    check(
        assert_type(b / left_i, "npt.NDArray[np.float64]"), pd.Series  # type: ignore[assert-type]
    )
    check(
        assert_type(i / left_i, "npt.NDArray[np.float64]"), pd.Series  # type: ignore[assert-type]
    )
    check(
