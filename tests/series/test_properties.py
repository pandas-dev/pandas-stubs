...
def test_property_dt() -> None:
    ...
    check(
        assert_type(df["ts"].dt, CombinedDatetimelikeProperties),
        DatetimeProperties,
    )
    check(
        assert_type(df["td"].dt, CombinedDatetimelikeProperties),
        TimedeltaProperties,
    )

    check(
        assert_type(df["ts"].dt.year, "Series[int]"),
        Series,
        np.integer,
    )
    check(
        assert_type(df["td"].dt.total_seconds(), "Series[float]"),
        Series,
        np.floating,
    )
