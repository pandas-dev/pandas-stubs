...
def test_scatter_matrix(close_figures: None) -> None:
    ...
    check(
        assert_type(pd.plotting.scatter_matrix(df, alpha=0.2), np_ndarray_object),
        np.ndarray,
    )
...
def test_table(close_figures: None) -> None:
    df = pd.DataFrame(np.random.randn(1000, 4), columns=["A", "B", "C", "D"])
    # TODO: matplotlib/matplotlib#31003 fig_kw in plt.subplots is untyped, hence the pyright ignore
...
def test_plot_pie(close_figures: None) -> None:
    ...
    check(
        assert_type(
            IRIS_DF.plot.pie(y="SepalLength", subplots=True), np_ndarray_object
        ),
        np.ndarray,
    )
...
def test_plot_pie_no_ylabel(close_figures: None) -> None:
...
def test_plot_kde_weights(close_figures: None) -> None:
...
def test_plot_df_scatter_color(close_figures: None) -> None:
