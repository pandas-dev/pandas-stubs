...
def test_types_drop() -> None:
    s = pd.Series([0, 1, 2])
    check(assert_type(s.drop(0), "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(s.drop([0, 1]), "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(s.drop(0, axis=0), "pd.Series[int]"), pd.Series, np.intp)
    ...
    # GH 302
    s = pd.Series([0, 1, 2])
    check(assert_type(s.drop(pd.Index([0, 1])), "pd.Series[int]"), pd.Series, np.intp)
    check(
...
def test_types_sort_index() -> None:
    ...
    check(assert_type(s.sort_index(), "pd.Series[int]"), pd.Series, np.intp)
    check(
        assert_type(s.sort_index(ascending=False), "pd.Series[int]"), pd.Series, np.intp
    )
...
def test_types_sort_values() -> None:
    ...
    check(assert_type(s.sort_values(axis=0), "pd.Series[int]"), pd.Series, np.intp)
    check(
...
def test_types_agg() -> None:
    ...
    check(assert_type(s.agg(["min", "max"]), "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(s.agg({"a": "min"}), "pd.Series[int]"), pd.Series, np.intp)
    check(
        assert_type(  # type: ignore[assert-type]
            s.agg("mean", axis=0), float  # pyright: ignore[reportGeneralTypeIssues]
        ),
        np.float64,
    )
    with pytest_warns_bounded(
        ...
        check(assert_type(s.agg(min), int), np.int64)
        check(assert_type(s.agg([min, max]), "pd.Series[int]"), pd.Series, np.intp)
        check(assert_type(s.agg({0: min}), "pd.Series[int]"), pd.Series, np.intp)
        check(
            assert_type(  # type: ignore[assert-type]
                s.agg(
...
def test_types_aggregate() -> None:
    ...
    check(
        assert_type(s.aggregate(["min", "max"]), "pd.Series[int]"), pd.Series, np.intp
    )
    check(assert_type(s.aggregate({"a": "min"}), "pd.Series[int]"), pd.Series, np.intp)
    with pytest_warns_bounded(
        FutureWarning,
        ...
        check(assert_type(s.aggregate({0: min}), "pd.Series[int]"), pd.Series, np.intp)


def test_types_transform() -> None:
    s = pd.Series([1, 2, 3], index=["col1", "col2", "col3"])
    check(assert_type(s.transform("abs"), "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(s.transform(abs), "pd.Series[int]"), pd.Series, np.intp)
...
def test_types_rename() -> None:
    ...
    check(assert_type(s3, "pd.Series[int]"), pd.Series, np.intp)
    ...
    check(assert_type(s4, "pd.Series[int]"), pd.Series, np.intp)

    # Dictionary
    s5 = pd.Series([1, 2, 3]).rename({1: 10})
    check(assert_type(s5, "pd.Series[int]"), pd.Series, np.intp)
    # inplace
...
def test_types_bfill() -> None:
    s1 = pd.Series([1, 2, 3])
    check(assert_type(s1.bfill(), "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(s1.bfill(inplace=False), "pd.Series[int]"), pd.Series, np.intp)
...
def test_types_ffill() -> None:
    s1 = pd.Series([1, 2, 3])
    check(assert_type(s1.ffill(), "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(s1.ffill(inplace=False), "pd.Series[int]"), pd.Series, np.intp)
...
def test_series_index_isin() -> None:
    ...
    check(assert_type(t1, "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(t2, "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(t3, "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(t4, "pd.Series[int]"), pd.Series, np.intp)
...
def test_series_invert() -> None:
    ...
    s3 = pd.Series([1, 2, 3])
    check(assert_type(s3[s2], "pd.Series[int]"), pd.Series, np.intp)
    check(assert_type(s3.loc[s2], "pd.Series[int]"), pd.Series, np.intp)
...
def test_reset_index() -> None:
    ...
    check(assert_type(r4, "pd.Series[int]"), pd.Series, np.intp)
    r5 = s.reset_index(["ab"], drop=True)
    check(assert_type(r5, "pd.Series[int]"), pd.Series, np.intp)
    r6 = s.reset_index(["ab"], drop=True, allow_duplicates=True)
    check(assert_type(r6, "pd.Series[int]"), pd.Series, np.intp)
...
def test_types_replace() -> None:
    # GH 44
    s = pd.Series([1, 2, 3])
    check(assert_type(s.replace(1, 2), "pd.Series[int]"), pd.Series, np.intp)
    check(
...
def test_prefix_summix_axis() -> None:
    s = pd.Series([1, 2, 3, 4])
    check(
        assert_type(s.add_suffix("_item", axis=0), "pd.Series[int]"), pd.Series, np.intp
    )
    check(
    ...
    check(
        assert_type(s.add_prefix("_item", axis=0), "pd.Series[int]"), pd.Series, np.intp
    )
    check(
...
def test_loc_callable() -> None:
    # GH 586
    s = pd.Series([1, 2])
    check(assert_type(s.loc[lambda x: x > 1], "pd.Series[int]"), pd.Series, np.intp)
