...
def test_rolling_basic_math() -> None:
    ...
    check(assert_type(DF.rolling(10).cov(), DataFrame), DataFrame)
    check(assert_type(DF.rolling(10).corr(), DataFrame), DataFrame)
...
def test_rolling_aggregate_series() -> None:
    ...
    check(assert_type(S.rolling(10).aggregate(["mean", np.mean]), DataFrame), DataFrame)
    check(
        assert_type(
            S.rolling(10).aggregate({"col1": "mean", "col2": np.mean}), DataFrame
        ),
        DataFrame,
    )
